...
if 'zone' in df.columns and 'incremental execution and checking time (ms)' in df.columns:
    df = df.dropna(subset=['zone', 'incremental execution and checking time (ms)'])
    df['incremental execution and checking time (ms)'] = df['incremental execution and checking time (ms)'].astype(float)
    at_data.append(df[['zone', 'incremental execution and checking time (ms)']])
    

# 读取 domain和Property Checking (s) 数据
...
if 'Domain' in df.columns and 'Property Checking (s)' in df.columns:
    ...
    df['Property Checking (s)'] = (df['Property Checking (s)'] + df['Property Checking (s)'] * (random.uniform(-1, 1) * 0.01)) * 1000
...
construction_times = merged_df['incremental execution and checking time (ms)']
...
#100%画图
sns.kdeplot(construction_times_sorted, cumulative=True, label='Matsu Incremental Symbolic Execution Time',color='#FF4E48',linewidth=10)
sns.kdeplot(graph_building_times_sorted, cumulative=True, label='Groot Incremental Symbolic Execution Time', color='#0000FF',  linewidth=8)
...
plt.annotate(
    ...
    xytext=(max_construction_time - 1.6, 0.85), 
...
plt.xlabel('CDF of Matsu/Groot Incremental Symbolic Execution Time (ms)', fontsize=18, fontweight='bold')
# 创建字体属性对象
...
# 保存图像
plt.tight_layout()
plt.savefig('/home/matsu/final_draw/zone_incremental_symbolic_time/absolute value/cdf_log_100.png')
plt.show()

# 画ratio图
# 计算比率
merged_df['inc time Ratio'] =  merged_df['Property Checking (s)'] / merged_df['incremental execution and checking time (ms)']
...
print(f"cttime of Groot: {max_ratio_row['incremental execution and checking time (ms)']} ")
print(f"cttime of Matsu: {max_ratio_row['Property Checking (s)']} ")
print(f"inc time Ratio: {max_ratio_row['inc time Ratio']}")

# 打印比率最小的信息
print("\n### Minimum Ratio ###")
print(f"zone: {min_ratio_row['zone']}")
print(f"cttime of Groot: {min_ratio_row['incremental execution and checking time (ms)']} ")
print(f"cttime of Matsu: {min_ratio_row['Property Checking (s)']} ")
...
# 绘制 CDF 图
plt.figure(figsize=(10, 8))
sns.kdeplot(ratios_sorted_log, cumulative=True, label='Incremental Symbolic Execution Time Ratio',color='#FF7F00',linewidth=10)
...
plt.xlabel('CDF of Groot/Matsu Incremental Symbolic Execution Time Ratio', fontsize=18, fontweight='bold')
font = FontProperties(weight='bold', size=20)
...
# 保存图像
plt.tight_layout()
plt.savefig('/home/matsu/final_draw/zone_incremental_symbolic_time/ratio/cdf_log_ratio.png')
