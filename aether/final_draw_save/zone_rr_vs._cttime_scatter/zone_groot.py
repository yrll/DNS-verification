...
file_path_cttime = '/home/matsu/attributes.csv'
...
if 'Domain' in df.columns and 'Total time (s)' in df.columns:
    ...
    ct_data.append(df[['Domain', 'Total time (s)']])
...
merged_df = pd.merge(merged_cnt_rr_df, ct_df, left_on='Domain', right_on='Domain', how='inner')
...
cttime = merged_df['Total time (s)'].dropna()
...
# print(f"cttime: {max_matsu_row['Total time (s)']} ms")
...
# 设置标题和轴标签，字体较大且加粗
plt.title('Scatter Plot of Groot(all) vs. Cttime', fontsize=30, fontweight='bold')
...
plt.gca().yaxis.set_major_locator(MaxNLocator(integer=True, prune='both', min_n_ticks=4, steps=[1, 2, 5, 10]))
...
# 保存图像
plt.savefig('/home/matsu/final_draw/rr_vs._cttime_scatter/groot_all_log.png')
