...
file_path_cttime = '/home/matsu/final_v1.1/construction_time_and_memory/groot_zone_ct_mem.csv'
...
if 'zone' in df.columns and 'total time (ms)' in df.columns:
    ...
    ct_data.append(df[['zone', 'total time (ms)']])
...
cttime = merged_df['total time (ms)'].dropna()
...
print(f"cttime: {max_matsu_row['total time (ms)']} ms")

# if len(rrnum_log) > len(cttime_log):
#     rrnum_log = rrnum_log[:len(cttime_log)]  
...
# 设置标题和轴标签，字体较大且加粗
plt.title('Scatter Plot of Groot RRs(cnt>100) vs. Simu-cttime', fontsize=26, fontweight='bold')
...
# 保存图像
plt.savefig('/home/matsu/final_draw/rr_vs._cttime_scatter/groot_simu_>100_log.png')
