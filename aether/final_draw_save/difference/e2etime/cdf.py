...
# 设置文件夹路径
file_path_Matsu = '/home/matsu/final_v1.1/difference/end_to_end_time_difference.csv'
...
if 'zone' in df.columns and 'matsu time' in df.columns and 'Total time (s)' in df.columns:
    df = df.dropna(subset=['zone', 'Total time (s)','matsu time'])
    df['matsu time'] = df['matsu time'] 
    mt_data.append(df[['zone', 'Total time (s)','matsu time']])
...
# 将数据转换为 NumPy 数组并排序
construction_times = mt_df['matsu time'].dropna()
graph_building_times = mt_df['Total time (s)'].dropna()
...
# print("Original construction_times (min, max):", construction_times.min(), construction_times.max())
# print("Log-transformed construction_times (min, max):", construction_times_log.min(), construction_times_log.max())
...
# #100%画图
# sns.kdeplot(construction_times_sorted, cumulative=True, bw_adjust=0.5, label='Matsu End-to-End Time',
#             color='#FF4E48', linewidth=10, clip=(min_construction_time, max_construction_time))
# sns.kdeplot(graph_building_times_sorted, cumulative=True, bw_adjust=0.5, label='Groot End-to-End Time',
...
#     xytext=(max_construction_time - 1.65, 0.8), 
...
# plt.xlabel("CDF of Large Zone's End-to-End Time (ms)", fontsize=26, fontweight='bold')
# # 创建字体属性对象
...
# # 保存图像
# plt.tight_layout()
# plt.savefig('/home/matsu/final_draw/difference/e2etime/cdf_log_100.png')
# plt.show()