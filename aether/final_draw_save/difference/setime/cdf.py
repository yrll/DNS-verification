...
# 设置文件夹路径
file_path_Matsu = '/home/matsu/final_v1.1/difference/symbolic_and_checking_difference.csv'
...
if 'zone' in df.columns and 'symbolic execution and properties checking time (ms)' in df.columns and 'Property Checking (s)' in df.columns:
    df = df.dropna(subset=['zone', 'Property Checking (s)','symbolic execution and properties checking time (ms)'])
    df['symbolic execution and properties checking time (ms)'] = df['symbolic execution and properties checking time (ms)'] 
    mt_data.append(df[['zone', 'Property Checking (s)','symbolic execution and properties checking time (ms)']])
...
# 将数据转换为 NumPy 数组并排序
construction_times = mt_df['symbolic execution and properties checking time (ms)'].dropna()
graph_building_times = mt_df['Property Checking (s)'].dropna()
...
# print("Original graph_building_times (min, max):", graph_building_times.min(), graph_building_times.max())
# print("Log-transformed graph_building_times (min, max):", graph_building_times_log.min(), graph_building_times_log.max())
...
# #100%画图
# sns.kdeplot(construction_times_sorted, cumulative=True, label='Matsu Symbolic Execution Time',
#             color='#FF4E48', linewidth=10, clip=(min_construction_time, None))
# sns.kdeplot(graph_building_times_sorted, cumulative=True, label='Groot Symbolic Execution Time',
#             color='#0000FF', linewidth=9, clip=(min_graph_building_time, None))
...
# min_graph_building_time_ori = graph_building_times_sorted_ori[0]
...
#     xytext=(max_construction_time - 1.6, 0.8), 
...
#     xy=(max_graph_building_time + 0.6, 1.0),
#     xytext=(max_graph_building_time , 0.55),
#     arrowprops=dict(facecolor='black', arrowstyle="->", linewidth=2),
#     fontsize=30, 
#     fontweight='bold',
#     ha='center'
...
# plt.xlabel("CDF of Large Zone's Symbolic Execution Time (ms)", fontsize=22, fontweight='bold')
# # 创建字体属性对象
...
# # 保存图像
# plt.tight_layout()
# plt.savefig('/home/matsu/final_draw/difference/setime/cdf_log_100.png')
# plt.show()