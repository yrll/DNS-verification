...
# 计算总时间（以分钟为单位）
matsu_max = at_df['construction time (ms)'].max() / 60000
matsu_total = at_df['construction time (ms)'].sum() / 60000
groot_total = merged_df['Graph building (s)'].sum() / 60000  # 转换为分钟
# groot_simu = groot_total / 80  # 转换为分钟


print(f"Matsu Total Construction Time: {matsu_total:.2f} minutes")

max_matsu_row = at_df.loc[at_df['construction time (ms)'].idxmax()]
max_groot_row = merged_df.loc[merged_df['Graph building (s)'].idxmax()]

# 打印计算出的总时间（以分钟为单位）
print(f"Total Matsu Construction Time: {max_matsu_row['construction time (ms)']:.2f} ")
print(f"Matsu Max Zone: {max_matsu_row['zone']}")
print(f"Total Groot Graph Building Time: {max_groot_row['Graph building (s)']:.2f}")
print(f"Groot Max Zone: {max_groot_row['zone']}")
...
# values = [matsu_max, groot_total, groot_simu]  # 转换 Matsu 时间为秒，其他时间已经是分钟
...
# # 保存图像
# output_file = '/home/matsu/final_draw/cttime_bar/construction_time_comparison_minutes_80.png'
# plt.tight_layout()
