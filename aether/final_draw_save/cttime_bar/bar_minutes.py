...
si_df = pd.concat(lg_data, ignore_index=True)

# 根据 "zone" 列对齐数据
merged_df_at_lg = pd.merge(
    at_df,
    lg_df,
    left_on='zone',
    right_on='Domain',
...
# 计算总时间（以分钟为单位）
matsu_total = merged_df['construction time (ms)'].max()
groot_total = merged_df['Graph building (s)'].sum() / 60000  # 转换为分钟
groot_simu = building_label_graph_sum / 80  # 转换为分钟

# 打印计算出的总时间（以分钟为单位）
print(f"Total Graph Building Time: {building_label_graph_sum:.2f} minutes")
print(f"Total Simulation Time: {generating_lecs_sum:.2f} minutes")
