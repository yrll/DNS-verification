...
if 'memory upper bound (bytes)' in df.columns:
    df = df.dropna(subset=['memory upper bound (bytes)'])
    times = df['memory upper bound (bytes)'] / (1024*1024)
...
#100%画图
sns.kdeplot(construction_times_sorted, cumulative=True, label='Matsu Memory Size',color='#FF4E48',linewidth=5)
sns.kdeplot(graph_building_times_sorted, cumulative=True, label='Groot Memory Size', color='#0000FF',  linewidth=3)
...
plt.annotate(
    ...
    xytext=(max_construction_time - 1.2, 0.85), 
...
plt.annotate(
    ...
    xytext=(max_graph_building_time - 0.9, 0.7),
    arrowprops=dict(facecolor='black', arrowstyle="->"),
    fontsize=26, 
    fontweight='bold',
    ha='center'
...
# 保存图像
plt.tight_layout()
plt.savefig('/home/matsu/final_draw/memory/absolute value/cdf_upper_log_100.png')
plt.show()